...
public class GithubLanguageService {
     ...
     */
    public List<LanguageStatsDto> fetchLanguagePercentages(String username) {
        ...
        return bytesByLang.entrySet().stream()
                .map(entry -> new LanguageStatsDto(
                        entry.getKey(),  // 언어 이름
                        entry.getValue(),  // 누적 바이트 수
                        Math.round(entry.getValue() * 10000.0 / totalBytes) / 100.0 // 사용 비율(소수점 둘째 자리까지 반올림)
                ))
